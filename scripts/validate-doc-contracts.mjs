...
const stalePathPatterns = [
  ...
  { pattern: /\.codex\/stacks\/react\.md\b/, expected: ".codex/rules/stacks/react.md" },
...
const activeVueBlockPatterns = [
  { pattern: /\bstack:\s*vue\b/i, message: "Unsupported stack `vue`. Use `stack: react`." },
  { pattern: /react\|vue/, message: "Combined stack contract is not allowed. Use React-only wording." },
  { pattern: /shadcn-vue/i, message: "Vue UI baseline is not allowed in active workflow files." },
  { pattern: /\bmotion-v\b/i, message: "Vue motion baseline is not allowed in active workflow files." },
  { pattern: /\bnuxt\b/i, message: "Nuxt/Vue references are not allowed in active workflow files." },
  { pattern: /\/stacks\/vue\.md\b/i, message: "Vue stack profile reference is not allowed in active workflow files." },
];
...
function isActiveWorkflowPath(relPath) {
  return !relPath.startsWith(".codex/legacy/");
}
...
for (const relPath of docPaths) {
  ...
  lines.forEach((line, index) => {
    ...
    if (isActiveWorkflowPath(relPath)) {
      for (const blockedPattern of activeVueBlockPatterns) {
        if (blockedPattern.pattern.test(line)) {
          addIssue(relPath, lineNumber, blockedPattern.message);
        }
      }
    }
  });
...
const pageContent = readRequiredFile("src/app/page.tsx");
if (pageContent) {
