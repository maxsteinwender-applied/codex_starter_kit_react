...
4. Start a new project with the mandatory kickoff gate:
...
Optional (for backend-enabled features):
...
| Type safety | `npm run typecheck` | TypeScript type checking passes |
...
| Combined check | `npm run check` | Lint + build pass together |
...
Fix: include `stack: react` when `project_track: coding`.
...
- Stack profile: `.codex/rules/stacks/STACKS.md`, `.codex/rules/stacks/react.md`
...
1. Add new features as route segments under `src/app/`.
2. Keep reusable UI in `src/components/` and utility logic in `src/lib/`.
3. If a feature depends on external services, document keys in `.env.example` and gate usage in code.
