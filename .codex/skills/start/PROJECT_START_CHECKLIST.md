...
4. If `project_track: coding` and stack is missing, stop and ask for `stack: react`.
5. If `project_track: operations`, stack must not be enforced.
6. If `project_track: coding` and a non-react stack is provided, stop and ask for `stack: react`.
